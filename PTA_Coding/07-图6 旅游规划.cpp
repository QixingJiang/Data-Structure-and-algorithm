...
struct GNode
{
    int Nv;
    int Ne;
    WeightType G[MaxVertexNum][MaxVertexNum];	// 放distance  (Weight)
...
Vertex FindMinDist(MGraph Graph, int dist[], int collected[]) {
    ...
    if (MinDist < INFINITY) /* 若上面找到了dist[V]，即找到最小dist */
        return MinV; /* 返回对应的顶点下标 */
    else
        return ERROR;  /* 若这样的顶点不存在，返回错误标记 */
}

//对建完的图Graph做Dijkstra算法 
bool Dijkstra(MGraph Graph, int dist[], int cost[], int path[], Vertex S)  //传入的S为源点
{
    int collected[MaxVertexNum];   //放置已收集点
    Vertex V, W;
        ...
        for (W = 0; W < Graph->Nv; W++) { // 对图中的每个顶点W
            if (collected[W] == false && Graph->G[V][W] < INFINITY)  // V的每个未被访问过的邻接点W
            {
                if (Graph->G[V][W] < 0) /* 若有负边 */
                    return false; /* 不能正确解决，返回错误标记 bool Dijkstra =false   */

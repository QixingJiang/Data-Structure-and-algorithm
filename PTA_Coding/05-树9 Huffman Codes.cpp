...
map<char, int> mappp;  // 用mappp这个map类型数据保存字符到频率的映射关系 
...
HuffmanTree createHuffman() {
	...
	Huff->weight = MinData;   // 初始化成频率最小 0
	Huff->left = NULL;
	Huff->right = NULL;
	return Huff;
}
...
int WPL(HuffmanTree Huff, int depth) {     //wpl的定义是权值*路径长度求和
	// 如果是叶结点，返回编码长度 
	if (Huff->left == NULL && Huff->right == NULL)
		return depth * Huff->weight; 
	else  // 否则返回其左右子结点的编码长度 
		return (WPL(Huff->left, depth + 1) + WPL(Huff->right, depth + 1));   //子结点的depth肯定多1 很合理
}
